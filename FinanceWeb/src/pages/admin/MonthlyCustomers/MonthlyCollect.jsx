...
export const MonthlyCollect = () => {
  ...
  // Helper to generate month-wise installment logs for a loan card (with OVERDUE detection)
  const getLoanInstallments = (loan) => {
    ...
    if (instList.length > 0) {
      return instList.map((inst, idx) => {
        ...
        const amt = Number(inst.amount || inst.scheduled_amount || loan.installment_amount || customer?.monthly_emi || 0);
        const paidAmt = Number(inst.paid_amount || (status === 'PAID' ? amt : 0));

        return {
          month_number: inst.installment_no || inst.month_number || (idx + 1),
          installment_number: inst.installment_no || inst.month_number || (idx + 1),
          due_date: dueStr,
          amount: amt,
          paid_amount: paidAmt,
          status,
          ...
          receipt_no: inst.receipt_no || (status === 'PAID' ? `REC-MTH-${customer?.id || '101'}-${inst.installment_no || idx + 1}` : null),
          payment_mode: inst.payment_mode || ((idx + 1) % 2 === 0 ? 'CASH' : 'UPI'),
          remaining_after: Math.max(
            0,
            (loan.total_repayment_amount || ((loan.installment_amount || amt) * (loan.total_installments || 12))) -
    ...
    const monthlyAmt = Number(loan.installment_amount || customer?.monthly_emi || 5000);
    const baseDateStr = toCleanIsoDate(loan.first_due_date || loan.issue_date || loan.start_date, '2026-09-15');
  ...
  const fetchCustomerData = async (isInitial = true) => {
    ...
    try {
      const list = await api.getMonthlyCustomers({
      ...
      if (found) {
        ...
        if (cust) {
          ...
          const monthlyLoans = rawLoans.filter((l) => (l.repayment_frequency || '').toUpperCase() === 'MONTHLY' || !l.repayment_frequency);
          const activeLoanList = (monthlyLoans.length > 0 ? monthlyLoans : rawLoans).map((l) => {
            const instList = l.installments || l.schedule || [];
            const totalInst = l.total_installments || (instList.length > 0 ? instList.length : 12);
            ...
            return {
              id: l.id,
              loan_code: l.loan_number || `LN-MTH-${cust.customer_code || cust.id}`,
              loan_name: l.loan_title || l.product_name || `${totalInst}-Month EMI Scheme`,
              principal: Number(l.principal_amount || 50000),
              interest_rate: Number(l.interest_rate || 24),
              total_repayment_amount: totalRepay,
              installment_amount: schedAmt || 5000,
              total_installments: totalInst,
              paid_installments: paidInst,
              remaining_balance: remaining,
              status: l.status,
              issue_date: l.disbursement_date ? String(l.disbursement_date).slice(0, 10) : '2026-09-15',
          ...
          const mapped = {
            ...
            monthly_emi: firstLoan?.installment_amount || 5000,
            current_month_status: firstLoan?.paid_installments > 0 ? 'PAID' : 'PENDING',
            total_installments: firstLoan?.total_installments || 12,
            paid_installments: firstLoan?.paid_installments || 0,
            outstanding_balance: firstLoan?.remaining_balance || 45000,
      ...
      console.error('Error retrieving monthly collection record:', err);
    } finally {
  ...
  // Open Monthly Log Modal for a specific loan card
  ...
  const totalPayableAmount = payableSelectedLoans.reduce((sum, l) => {
    const amt = Number(l.installment_amount || customer?.monthly_emi || 5000);
    return sum + amt;
  }, 0);

  // Total monthly due for this borrower across all active loans
  const totalCombinedMonthlyDue = activeLoans.reduce((sum, l) => sum + Number(l.installment_amount || customer?.monthly_emi || 5000), 0);
    ...
    relevant.forEach((loan) => {
      ...
      installments.forEach((inst) => {
        if (inst.status === 'OVERDUE') {
          overdueCount++;
          overdueTotalAmount += Number(inst.amount || loan.installment_amount || 5000);
  ...
  const handleSubmitPayment = async (e) => {
    ...
    try {
      ...
      for (const loan of payableSelectedLoans) {
        const amt = loan.installment_amount || customer?.monthly_emi || 5000;
        ...
        const receiptNo = res?.paymentNumber || res?.receipt_no || res?.receiptNumber || `REC-MTH-${Date.now().toString().slice(-6)}`;
        collectedItems.push({
          loan_id: loan.id,
      ...
      <div style={{ maxWidth: 740, margin: '1.5rem auto', padding: '0 1rem' }}>
        ...
        >
          ...
          <h2 style={{ margin: '0 0 0.4rem 0', color: 'var(--text-primary)', fontSize: '1.65rem', fontWeight: 900, letterSpacing: '-0.02em' }}>
            Monthly Collection Receipt
          </h2>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '1.75rem' }}>
            Monthly EMI installment repayment successfully logged into branch portfolio.
          </p>
          ...
          >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem', marginBottom: '1rem' }}>
              ...
              <div>
                <span style={{ color: 'var(--text-muted)', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase' }}>Profession / Sector</span>
                <div style={{ color: 'var(--text-secondary)', fontWeight: 700, fontSize: '0.88rem' }}>
                  {receiptData.occupation || 'Salaried Professional'}
                </div>
              </div>
            ...
            <div style={{ marginBottom: '0.85rem' }}>
              <span style={{ fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--text-muted)', display: 'block', marginBottom: '0.5rem' }}>
                Allocated Monthly EMI Installments:
              </span>
  ...
  return (
    <div style={{ width: '100%', maxWidth: '100%', padding: '0 0.5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '0.75rem' }}>
        ...
        {/* Tab Selector: Collection Mode vs Month-wise Installment Schedule */}
        <div style={{ display: 'flex', background: '#F1F5F9', padding: '0.25rem', borderRadius: 8, border: '1px solid #E2E8F0' }}>
          ...
          >
            <Calendar size={14} /> Month-Wise Ledger & Schedule
          </button>
        </div>
      </div>
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '0.85rem', marginTop: 4, fontSize: '0.82rem', color: 'var(--text-secondary)', flexWrap: 'wrap' }}>
                ...
                <span>• Profession: <strong style={{ color: 'var(--text-primary)', fontWeight: 600 }}>{customer.occupation || 'Salaried'}</strong></span>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '1.25rem', background: '#F8FAFC', padding: '0.75rem 1.25rem', borderRadius: 8, border: '1px solid #E2E8F0' }}>
            <div>
              <span style={{ fontSize: '0.68rem', color: 'var(--text-muted)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em', display: 'block' }}>Monthly Combined EMI</span>
      ...
      {activeTab === 'COLLECT' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
                Active Monthly Loan Schemes ({activeLoans.length})
              </h3>
            ...
            {activeLoans.map((loan, idx) => {
              ...
              const paidMonths = paidInstallmentsList.length;
              const totalMonths = loanInstallments.length || loan.total_installments || 12;
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                    <div style={{ display: 'flex', gap: '0.65rem', alignItems: 'center' }}>
                      ...
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', flexWrap: 'wrap' }}>
                          ...
                          >
                            {loan.loan_name || `${totalMonths}-Month Scheme`}
                          </span>
                        </div>
                        <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', fontWeight: 600 }}>
                          Issued: {loan.issue_date || '2026-09-15'} • Maturity: {loan.maturity_date || '2027-09-15'}
                        </span>
                        {isOverdue && (
                          <span
                            style={{
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <span style={{ fontSize: '0.68rem', color: 'var(--text-muted)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em', display: 'block' }}>Monthly EMI</span>
                      <strong style={{ fontSize: '1.35rem', fontWeight: 900, color: isPaidForToday ? '#047857' : 'var(--primary)', letterSpacing: '-0.02em' }}>
                        {formatCurrency(loan.installment_amount || customer?.monthly_emi || 5000)}
                      </strong>
                    </div>
                  </div>
                  ...
                  >
                    <div>
                      ...
                      <strong style={{ color: 'var(--text-primary)', fontWeight: 800 }}>
                        {formatCurrency(loan.principal)} @ {loan.interest_rate || 24}%
                      </strong>
                    </div>
                    <div>
                  ...
                  >
                    ...
                    >
                      <Calendar size={13} /> View Monthly Log Modal ↗
                    </button>
                    ...
                    {(() => {
                      ...
                      if (isOverdue) {
                        return (
                          ...
                          >
                            <AlertTriangle size={13} color="#DC2626" /> {overdueCount} Overdue — {formatCurrency(loan.installment_amount || customer?.monthly_emi || 5000)}/mo
                          </span>
                        );
                      }
                      ...
                      return (
                        ...
                        >
                          Due on {collectionDate}: {formatCurrency(loan.installment_amount || customer?.monthly_emi || 5000)}
                        </span>
                      );
                    })()}
                  </div>
          ...
          {/* Right Column: Record Monthly Payment Panel */}
          <div
            className="card"
            style={{
              padding: '1.75rem',
          ...
          >
            ...
            <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginBottom: '1.25rem' }}>
              Collect dues for selected date across borrower's active monthly EMI schemes.
            </p>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem' }}>
                    ...
                    <span style={{ fontSize: '0.76rem', color: '#991B1B', fontWeight: 700 }}>
                      {overdueSummary.overdueCount} missed month{overdueSummary.overdueCount > 1 ? 's' : ''} ({formatCurrency(overdueSummary.overdueTotalAmount)})
                    </span>
                  </div>
            ...
            {isBeforeLoanTenure ? (
              ...
              >
                ...
                <p style={{ color: '#1E3A8A', fontSize: '0.85rem', margin: '0 0 1.25rem 0', fontWeight: 600 }}>
                  Selected date ({collectionDate}) is before the first scheduled installment due date. Month 1 collection starts on <strong>{loanDateBounds.minDate}</strong>.
                </p>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', flexWrap: 'wrap' }}>
                  ...
                  >
                    Go to Month 1 ({loanDateBounds.minDate}) →
                  </button>
                  <button
                    type="button"
                    className="btn btn-secondary"
              ...
              >
                ...
                <p style={{ color: '#047857', fontSize: '0.85rem', margin: '0 0 1.25rem 0', fontWeight: 600 }}>
                  All active loan installments ({formatCurrency(totalCombinedMonthlyDue)}) for this date are already collected and cleared.
                </p>
      ...
      {/* ===================================================================== */}
      {/* TAB 2: MONTH-WISE INSTALLMENT SCHEDULE & REPAYMENT LEDGER            */}
      {/* ===================================================================== */}
      {activeTab === 'SCHEDULE' && (
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '0.75rem' }}>
            <div>
              <h3 style={{ margin: 0, fontSize: '1.15rem', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
                Month-Wise Installment Schedule & Ledger
              </h3>
              <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                Select an active monthly loan scheme to view its complete date-of-payment breakdown
              </span>
            </div>
            ...
            <div>
              ...
              >
                ...
                <div>
                  ...
                  <strong style={{ color: 'var(--primary)', fontWeight: 800 }}>
                    {formatCurrency(currentScheduleLoan.principal)} @ {currentScheduleLoan.interest_rate || 24}%
                  </strong>
                </div>
                <div>
                  <span style={{ color: 'var(--text-muted)', display: 'block', fontSize: '0.68rem', fontWeight: 800, textTransform: 'uppercase' }}>Tenure</span>
                  <strong style={{ color: 'var(--text-primary)', fontWeight: 800 }}>
                    {currentScheduleLoan.total_installments || 12} Monthly Installments
                  </strong>
                </div>
                <div>
                  <span style={{ color: 'var(--text-muted)', display: 'block', fontSize: '0.68rem', fontWeight: 800, textTransform: 'uppercase' }}>Monthly EMI</span>
                  <strong style={{ color: '#047857', fontWeight: 900 }}>
                    {formatCurrency(currentScheduleLoan.installment_amount || customer?.monthly_emi || 5000)}/month
                  </strong>
                </div>
              </div>
                    ...
                    {getLoanInstallments(currentScheduleLoan).map((inst, i) => {
                      ...
                      return (
                        <tr key={i} style={{ background: isOverdueInst ? '#FFF5F5' : undefined }}>
                          <td>
                            <strong style={{ color: 'var(--text-primary)', fontWeight: 800 }}>
                              Month {inst.month_number || inst.installment_number || i + 1}
                            </strong>
                          </td>
                          <td>
                            <span style={{ color: 'var(--text-secondary)', fontWeight: 600 }}>
                              {inst.due_date || '2026-09-15'}
                            </span>
                          </td>
                          <td>
      ...
      {/* ===================================================================== */}
      {/* MONTHLY LOG MODAL (MATCHING SHOPKEEPER MODAL ARCHITECTURE)            */}
      {/* ===================================================================== */}
          ...
          >
            ...
            <div style={{ padding: '1.25rem 1.5rem' }}>
              {(() => {
                ...
                const totalCount = insts.length || Number(selectedLoanForModal.total_installments || 12);
                const monthlyAmt = Number(selectedLoanForModal.installment_amount || customer?.monthly_emi || 5000);
                const paidAmount = paidList.reduce((acc, i) => acc + Number(i.paid_amount || monthlyAmt), 0);
                  ...
                  <>
                    ...
                    >
                      <div style={{ background: '#F8FAFC', padding: '0.75rem 1rem', borderRadius: 8, border: '1px solid #E2E8F0' }}>
                        <span style={{ fontSize: '0.68rem', color: 'var(--text-muted)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em', display: 'block' }}>Monthly Installment</span>
                    ...
                    <div style={{ marginBottom: '1.25rem' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', marginBottom: 5 }}>
                        ...
                        <strong style={{ color: 'var(--text-primary)', fontWeight: 800 }}>
                          Month {paidCount} of {totalCount} ({progressPct}%)
                        </strong>
                      </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.85rem' }}>
                <span style={{ fontSize: '0.85rem', fontWeight: 800, color: 'var(--text-primary)' }}>
                  Month-by-Month Installment Ledger
                </span>
                <div style={{ display: 'flex', gap: '0.35rem' }}>
                      ...
                      .map((inst, idx) => {
                        ...
                        return (
                          <tr key={idx} style={{ background: isOverdueModal ? '#FFF5F5' : undefined }}>
                            <td>
                              <strong style={{ color: 'var(--text-primary)', fontWeight: 800 }}>
                                Month {inst.month_number || inst.installment_number || idx + 1}
                              </strong>
                            </td>
                            <td>
            ...
            >
              ...
              >
                <Printer size={15} /> Print Monthly Log
              </button>
